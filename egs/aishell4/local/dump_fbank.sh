...
if [ -f ${data}/segments ]; then
  ...
  ${cmd} JOB=1:${nj} ${logdir}/dump_fbank_${name}.JOB.log \
      dump-fbank.py ${opts} --filetype ${filetype} --verbose=${verbose} --compress=${compress} \
  ...
  ${cmd} JOB=1:${nj} ${logdir}/dump_fbank_${name}.JOB.log \
      local/dump-fbank.py ${opts} --filetype ${filetype} --verbose=${verbose} --compress=${compress} \
...
for n in $(seq ${nj}); do
  cat ${fbankdir}/raw_fbank_${name}.${n}.scp
done > ${data}/feats.scp
...
echo "Succeeded dumping fbank for ${name}"
