...
scoring_opts=
minimize=false
#model=model.epoch.26
model=best_model_aishell_unified_global_cmvn_predict_new_feat
#model=avg_56_10.pt
#model=avg_best_10.pt
...
# check files
num_error=0
for f in $TLG_dir/TLG.fst $graph/words.txt $input_scp $srcdir/config.json; do
  if [ ! -f "$f" ]; then
    echo "Missing file: $f"
...
if [ $stage -le 0 ]; then
  mkdir -p $logits
  python3 ctc-crf/calculate_logits_chunk_predict_multich.py \
    --nj=$nj \
    --input_scp=$input_scp \
