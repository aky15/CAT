...
class Postnet(nn.Module):
  ...
  def __init__(self, input_size, out_len, output_size=80):
    ...
    self.layer = nn.Linear(in_features=input_size, out_features=out_len*output_size)
...
class PredictNet(nn.Module):
  ...
  def __init__(self, mel_dim, out_len, hdim, rnn_num_layers, rnn_dropout, rnn_residual):
  ...
  def forward(self, inputs, chunk_size):
    ...
    # predicted_mel = self.postnet(h.squeeze(0))
    # predicted_mel: (batch_size, seq_len, mel_dim)
