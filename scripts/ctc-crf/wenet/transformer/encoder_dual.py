...
class BaseEncoderDual(torch.nn.Module):
    def __init__(
        self,
        input_size: int,
        output_size: int = 256,
    ...
    def forward(
        ...
        xs_offline, xs_online = xs, xs
        mask_pad = masks  # (B, 1, T/subsample_rate)
        
        online_masks = add_optional_chunk_mask(xs_online, masks,
        ...
        offline_masks = add_optional_chunk_mask(xs_offline, masks,
        ...
        for idx, layer in enumerate(self.encoders):
            xs_online, online_masks, _ = layer(xs_online, online_masks, pos_emb, mask_pad)
            xs_offline, offline_masks, _ = layer(xs_offline, offline_masks, pos_emb, mask_pad)    
            if idx == self.num_blocks // 2:
                xs_online_internal = xs_online
                xs_offline_internal = xs_offline        
            online_list.append(xs_online)
            offline_list.append(xs_offline)

        if self.normalize_before:
            xs_online = self.after_norm(xs_online)
            xs_offline = self.after_norm(xs_offline)
            xs_online_internal = self.after_norm(xs_online_internal)
            xs_offline_internal = self.after_norm(xs_offline_internal)

        # Here we assume the mask is not changed in encoder layers, so just
        # return the masks before encoder layers, and the masks will be used
        # for cross attention with decoder later
        return torch.cat(online_list), torch.cat(offline_list), xs_online_internal, xs_offline_internal, xs_online, xs_offline, masks
...
class TransformerEncoderDual(BaseEncoderDual):
    ...
    def __init__(
        ...
        global_cmvn: torch.nn.Module = None,
        use_dynamic_left_chunk: bool = True,
    ):
...
class ConformerEncoderDual(BaseEncoderDual):
