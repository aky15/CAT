...
if torch.__version__ > '1.0.0':
    ...
    from torchaudio.sox_effects import effect_names as get_effect_names
...
class EffectChain:
    ...
    def _apply_sox_effects(chain: List[SoxEffect],
        ...
        input_tensor.div_(EffectChain._NORMALIZER)
        #print(torch.max(input_tensor))
        #print(torch.min(input_tensor))
        #exit()
        ...
        out, sr = sox_chain.apply_flow_effects(input_tensor,
                                          src_info,
                                          target_info)
        out.mul_(EffectChain._NORMALIZER)
        #print(torch.max(out))
        #print(torch.min(out))
        #exit()
...
class TimeDropout:
    ...
    def __call__(self, x, src_info, dst_info):
        ...
        start = np.random.randint(0, max(1,x.size(1) - length))
        end = start + length
...
class AdditiveNoise:
    def __init__(self, std, lower=-20, upper=-5):
        self.std = std
        self.lower = lower
        self.upper = upper

    def __call__(self, x, src_info, dst_info):
        noise_instance = torch.normal(0, self.std, size=x.size())
        assert noise_instance.numel() == x.numel(
        ), 'Noise and signal shapes are incompatible'
        ratio = self.lower + ( self.upper - self.lower) * torch.rand(1)
        ratio = 10 ** (ratio / 20)
        scale = ratio * torch.sqrt((x ** 2).mean())
        noised = x + scale * noise_instance.view_as(x)
